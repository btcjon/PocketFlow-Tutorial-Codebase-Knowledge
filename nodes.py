...
def get_content_for_indices(files_data, indices, max_chars_per_file=6000):
    ...
    for i in indices:
        if 0 <= i < len(files_data):
            ...
            # Smart truncation - keep full content for smaller files, truncate large ones
            if len(content) > max_chars_per_file:
                # Keep beginning and end of large files for better context
...
class IdentifyAbstractions(Node):
    def prep(self, shared):
        ...
        def create_llm_context(files_data, max_chars_per_file=4000):
            ...
            for i, (path, content) in enumerate(files_data):
                # Smart truncation for the initial analysis
                if len(content) > max_chars_per_file:
                    half_limit = max_chars_per_file // 2
                    truncated_content = (
                        content[:half_limit] + 
    ...
    def exec(self, prep_res):
        ...
        print(f"Identifying abstractions using LLM ({llm_provider})...")

        # Set environment variable for the LLM call
...
class AnalyzeRelationships(Node):
    ...
    def exec(self, prep_res):
        ...
        print(f"Analyzing relationships using LLM ({llm_provider})...")
...
class MergeToSingleFile(Node):
    """Node to merge index.md and chapter files into a single tutorial file"""
    ...
    def exec(self, prep_res):
        ...
        print(f"Merging tutorial files into single document...")
        
        # Read index.md content
        index_path = os.path.join(output_dir, "index.md")
        if not os.path.exists(index_path):
            raise FileNotFoundError(f"index.md not found in {output_dir}")
        
        with open(index_path, "r", encoding="utf-8") as f:
            index_content = f.read()
        
        # Extract parts from index.md (everything except the chapter links section)
        lines = index_content.split("\n")
        header_lines = []
        chapter_links_started = False
        
        for line in lines:
            if line.strip() == "## Chapters":
                chapter_links_started = True
                break
        ...
        # Find and read chapter files in order
        chapter_files = glob.glob(os.path.join(output_dir, "[0-9][0-9]_*.md"))
        chapter_files.sort()  # Ensure correct order
        ...
        if not chapter_files:
                ...
                with open(chapter_file, "r", encoding="utf-8") as f:
                    ...
                    for line in content.split("\n"):
                        if line.strip().startswith("# Chapter"):
                            chapter_titles.append(line.strip()[2:])  # Remove "# "
                            break
            ...
            for i, title in enumerate(chapter_titles, 1):
                ...
                combined_content += f"{i}. [{title}](#{anchor})\n"
            
            combined_content += "\n---\n\n"
            
            # Second pass: add chapter content
            for chapter_file in chapter_files:
                print(f"  - Merging {os.path.basename(chapter_file)}")
                
                with open(chapter_file, "r", encoding="utf-8") as f:
                    chapter_content = f.read().strip()
                
                # Remove any existing attribution at the end of chapters
        ...
        # Add final attribution
        combined_content += "Generated by [AI Codebase Knowledge Builder](https://github.com/The-Pocket/Tutorial-Codebase-Knowledge)\n"
        
        # Create the single tutorial file with proper naming
        single_file_name = f"{project_name}_tutorial.md"
        single_file_path = os.path.join(output_dir, single_file_name)
        
        with open(single_file_path, "w", encoding="utf-8") as f:
            f.write(combined_content)
        
        print(f"  - Created {single_file_name}")
        ...
        try:
            ...
            print(f"  - Cleaned up {len(chapter_files) + 1} original files")
    ...
    def post(self, shared, prep_res, exec_res):
        print(f"Tutorial merged into single file in: {exec_res}")


class MoveToDocs(Node):
    """Node to move generated tutorial from output folder to docs folder"""
    ...
    def prep(self, shared):
        ...
        docs_dir = os.path.join("docs", project_name)
    ...
    def exec(self, prep_res):
        ...
        output_dir = prep_res["output_dir"]
        docs_dir = prep_res["docs_dir"]
        project_name = prep_res["project_name"]
        
        print(f"Moving tutorial from {output_dir} to {docs_dir}...")
        ...
        # Move the entire output directory to docs
        shutil.move(output_dir, docs_dir)
        
        print(f"Successfully moved tutorial to: {docs_dir}")
        return docs_dir
    
    def post(self, shared, prep_res, exec_res):
        shared["final_docs_dir"] = exec_res
        print(f"\nTutorial moved to docs folder: {exec_res}")
